app.controller('BloodTestRecordFormController', function($scope, $stateParams, SecurityApi, BloodTestRecord) {
  ...
  if($stateParams.did != 'new') {
    BloodTestRecord.get({id: $stateParams.did}, function(record) {
      fixRecord(record);
      $scope.record = record;
      detectChanges();
    });
    ...
    $scope.record = new BloodTestRecord();
